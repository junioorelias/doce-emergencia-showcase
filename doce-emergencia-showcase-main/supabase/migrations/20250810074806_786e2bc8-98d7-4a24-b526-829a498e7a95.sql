-- 1) Create roles enum, user_roles table, has_role function
...
-- Use SECURITY DEFINER helper policy for writes via a wrapper function
DO $$ BEGIN
...
-- 2) Tighten coupons and rewards RLS to admin for writes
-- Drop permissive existing policies that allow any authenticated user broad access
-- Note: Use IF EXISTS guards
DO $$ BEGIN
...
DO $$ BEGIN
  CREATE POLICY IF NOT EXISTS "Admins can view all coupons" ON public.coupons
  FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(),'admin'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY IF NOT EXISTS "Admins can insert coupons" ON public.coupons
  FOR INSERT TO authenticated
  WITH CHECK (public.has_role(auth.uid(),'admin'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY IF NOT EXISTS "Admins can update coupons" ON public.coupons
  FOR UPDATE TO authenticated
  USING (public.has_role(auth.uid(),'admin'))
  WITH CHECK (public.has_role(auth.uid(),'admin'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Keep public view of active coupons already present
...
DO $$ BEGIN
  CREATE POLICY IF NOT EXISTS "Admins can view all rewards" ON public.rewards
  FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(),'admin'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY IF NOT EXISTS "Admins can insert rewards" ON public.rewards
  FOR INSERT TO authenticated
  WITH CHECK (public.has_role(auth.uid(),'admin'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE POLICY IF NOT EXISTS "Admins can update rewards" ON public.rewards
  FOR UPDATE TO authenticated
  USING (public.has_role(auth.uid(),'admin'))
  WITH CHECK (public.has_role(auth.uid(),'admin'));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- 3) Create voting tables
...
DO $$ BEGIN
  CREATE POLICY "Public can view options of active polls" ON public.poll_options
  FOR SELECT TO anon, authenticated
  USING (
    EXISTS (
...
-- 4) Improve redeem functions to ensure profile exists
